  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      try {
        ...
        const emailSent = await EnhancedEmailService.sendEmailWithRetry('search', formData);
        ...
        if (emailSent) {
          ...
          console.warn('⚠️ Email de recherche ajouté à la queue pour retry');
          toast.warning('📧 Demande enregistrée - notification en cours de traitement', {
            duration: 4000
          });
        }
      } catch (emailError) {
        console.error('Erreur envoi notification RDV:', emailError);
        toast.warning('📧 Demande sauvegardée - retry automatique activé', {
          duration: 4000
        });
      }
