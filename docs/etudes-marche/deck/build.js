  ...
  const rows = [
    entetes.map((t) => ({
      ...
      options: {
        ...
        fill: { color: STONE }, valign: "middle", margin: [4, 7, 4, 7],
      },
    })),
  ];
  lignes.forEach((ligne) => {
    const fort = ligne.fort;
    rows.push((ligne.cells || ligne).map((t, i) => ({
      text: String(t),
      options: {
        fontFace: BODY, fontSize: o.fontSize || 10,
        bold: !!fort, color: fort ? INK : "3D362C",
        fill: { color: fort ? "F6EFE0" : WHITE },
        align: i >= (o.numFrom != null ? o.numFrom : 99) ? "right" : "left",
        valign: "middle", margin: [4, 7, 4, 7],
      },
    })));
  });
  s.addTable(rows, {
    x, y, w, colW: largeurs, border: { type: "solid", pt: 0.5, color: "E2DCCF" },
    autoPage: false,
  });
}
...
/* ══ 1. Couverture ═════════════════════════════════════════════════════ */
{
  ...
  s.addNotes("Villa neuve grand luxe, 421,82 m² intérieurs sur une parcelle de 2 033 m² à La Nartelle. Position perchée vue mer (+37,92 à +44,77 NGF) — ce n'est pas un pieds dans l'eau, et il ne faut jamais le laisser croire.");
}
...
/* ══ 2. Le bien en un regard ═══════════════════════════════════════════ */
{
  ...
  s.addNotes("Insister sur le ratio d'emprise : 19,6 % seulement, donc 1 635 m² d'extérieurs. Rare sur un secteur où la contrainte foncière pousse à la densification.");
}
...
/* ══ 4. Suites ═════════════════════════════════════════════════════════ */
{
  ...
  s.addNotes("C'est le retournement à tenir en rendez-vous. Ne jamais dire « chambres ».");
}
...
/* ══ 5. Prestations ════════════════════════════════════════════════════ */
{
  ...
  s.addNotes("L'acquéreur international dispose d'un actif exploitable dès la remise des clés, y compris en location saisonnière dès la première saison.");
}
...
slideSombre("20 000 €/m², est-ce défendable ?", "II · La démonstration",
  ...
  .addNotes("Transition. Le contradicteur ouvrira toujours sur la moyenne communale.");
...
/* ══ 9. Méthode 1 — décomposition ══════════════════════════════════════ */
{
  ...
  s.addChart(pres.ChartType.bar, [
    { name: "Coût de production hors foncier", labels: ["20 000 €/m²"], values: [11289] },
    { name: "Foncier implicite", labels: ["20 000 €/m²"], values: [5711] },
    { name: "Marge et frais financiers", labels: ["20 000 €/m²"], values: [3000] },
  ], {
    x: M, y: 1.34, w: 9, h: 1.72,
    barDir: "bar", barGrouping: "stacked",
    chartColors: [C1, C2, C3],
    ...
    showLegend: true, legendPos: "b", legendColor: MUTED, legendFontFace: BODY, legendFontSize: 9.5,
    catAxisHidden: true,
    valAxisLabelColor: MUTED, valAxisLabelFontFace: BODY, valAxisLabelFontSize: 9,
    valGridLine: { color: "EDE8DD", size: 0.5 }, catGridLine: { style: "none" },
    valAxisMaxVal: 20000, showTitle: false,
  });
  const w = 2.94, g = 0.19;
  const col = (i) => M + i * (w + g);
...
/* ══ 12. Méthode 2 — les annonces ══════════════════════════════════════ */
{
  ...
  s.addChart(pres.ChartType.bar, [{
    name: "€/m²",
    labels: [
      "Contemporaine vue mer, 295 m²", "Villa neuve centre, 320 m²",
      "Contemporaine neuve, 264 m²", "Surplombant la mer, 358 m²",
      "Méditerranéenne, 365 m²", "Vue mer except., 347 m²",
      "Bord de mer, 340 m²", "Provençale, 350 m²",
      "Vue mer, 350 m²", "Standard vue mer, 400 m²",
      "VILLA ROI DE LA CAMARGUE, 422 m²",
    ],
    values: [23390, 19688, 18182, 18156, 17808, 16398, 16176, 15714, 14857, 10625, 20000],
  }], {
    x: M, y: 1.3, w: 9, h: 3.35,
    barDir: "bar", chartColors: [C1, C1, C1, C1, C2, C2, C2, C2, C2, C3, INK],
    showValue: true, dataLabelPosition: "outEnd", dataLabelColor: "3D362C",
    dataLabelFontFace: BODY, dataLabelFontSize: 8.5, dataLabelFormatCode: '#,##0 "€"',
    showLegend: false, showTitle: false,
    catAxisLabelColor: "3D362C", catAxisLabelFontFace: BODY, catAxisLabelFontSize: 8.5,
    valAxisHidden: true, valGridLine: { style: "none" }, catGridLine: { style: "none" },
    valAxisMaxVal: 27000, barGapWidthPct: 45,
  });
...
/* ══ 14. Méthode 3 — prime du neuf ═════════════════════════════════════ */
{
  ...
  s.addChart(pres.ChartType.bar, [{
    ...
    labels: ["Neuf et contemporain", "Ancien et rénové", "Standard vue mer"],
    values: [19854, 16191, 10625],
  }], {
    x: M, y: 1.34, w: 4.5, h: 2.5,
    barDir: "col", chartColors: [C1, C2, C3],
    showValue: true, dataLabelPosition: "outEnd", dataLabelColor: "3D362C",
    dataLabelFontFace: BODY, dataLabelFontSize: 10, dataLabelFormatCode: '#,##0 "€"',
    showLegend: false, showTitle: false,
    catAxisLabelColor: "3D362C", catAxisLabelFontFace: BODY, catAxisLabelFontSize: 9,
    valAxisHidden: true, valGridLine: { style: "none" }, catGridLine: { style: "none" },
    valAxisMaxVal: 23000, barGapWidthPct: 55,
  });
  s.addText("+ 22,6 %", {
...
/* ══ 15. La tranche 7–12 M€ ════════════════════════════════════════════ */
{
  ...
  s.addNotes("Point de vigilance à dire au vendeur : le vivier est étroit. Un mandat de trois mois n'a aucun sens sur cette tranche.");
}
...
/* ══ 18. Recommandation ════════════════════════════════════════════════ */
{
  ...
  s.addNotes("Ne pas descendre sous le plancher sans contrepartie — délai raccourci, mobilier conservé, conditions suspensives allégées.");
}
...
pres.writeFile({ fileName: "Villa-Roi-de-la-Camargue-defense-de-prix.pptx" })
